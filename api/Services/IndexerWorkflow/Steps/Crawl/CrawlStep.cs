using LiteralLifeChurch.ArchiveManagerApi.Exceptions.IndexerWorkflow.Crawl;
...
namespace LiteralLifeChurch.ArchiveManagerApi.Services.IndexerWorkflow.Steps.Crawl
{
    public class CrawlStep : IndexerWorkflowStep<object, Task<List<DriveItem>>, CrawlException>
    {
        public CrawlStep(ConfigurationModel config) : base(config)
        {
        }

        public async override Task<List<DriveItem>> Run(object item)
        {
        ...
        private async Task<List<DriveItem>> GetChildrenInAllSharedFolders()
        {
            ...
            LoggerService.Info($"Crawler returned {media.Count} item(s)", LoggerService.Crawl);
            return media;
        }
